...
parser.add_argument('--psm', type=int, choices=[0, 1, 2], default=0)
...
# Load dataset
grasp_obs = np.load(f'../data/{args.task}/grasp/obs_orn.npy')
grasp_acs = np.load(f'../data/{args.task}/grasp/acs_orn.npy')
handover_obs = np.load(f'../data/{args.task}/handover/obs_orn.npy')
handover_acs = np.load(f'../data/{args.task}/handover/acs_orn.npy')
release_obs = np.load(f'../data/{args.task}/release/obs_orn.npy')
release_acs = np.load(f'../data/{args.task}/release/acs_orn.npy')
...
if args.task == 'BiPegBoard-v0':
    grasp_acs[:, :, 0] *= np.deg2rad(15)
    grasp_acs[:, :, 1] *= np.deg2rad(30)
    handover_acs[:, :, 0] *= np.deg2rad(15)
    handover_acs[:, :, 1] *= np.deg2rad(30)
    release_acs[:, :, 0] *= np.deg2rad(15)
    release_acs[:, :, 1] *= np.deg2rad(30)
else:
    grasp_acs *= np.deg2rad(30)
    handover_acs *= np.deg2rad(30)
    release_acs *= np.deg2rad(30)

if args.psm == 1:
    grasp_obs = grasp_obs[:, :, :3]
    grasp_acs = grasp_acs[:, :, 0].reshape(
        num_episode, grasp_data_size, 1)
    handover_obs = handover_obs[:, :, :3]
    handover_acs = handover_acs[:, :, 0].reshape(
        num_episode, handover_data_size, 1)
    release_obs = release_obs[:, :, :3]
    release_acs = release_acs[:, :, 0].reshape(
        num_episode, release_data_size, 1)
elif args.psm == 2:
    grasp_obs = grasp_obs[:, :, 3:]
    grasp_acs = grasp_acs[:, :, 1].reshape(
        num_episode, grasp_data_size, 1)
    handover_obs = handover_obs[:, :, 3:]
    handover_acs = handover_acs[:, :, 1].reshape(
        num_episode, handover_data_size, 1)
    release_obs = release_obs[:, :, 3:]
    release_acs = release_acs[:, :, 1].reshape(
        num_episode, release_data_size, 1)
...
# grasp_t = torch.arange(0., 0.1 * grasp_data_size, 0.1).to(device)
# handover_t = torch.arange(0., 0.1 * handover_data_size, 0.1).to(device)
# release_t = torch.arange(0., 0.1 * release_data_size, 0.1).to(device)
...
def get_batch(bitr):  # get training data from bitr-th trajectory
    ...
    # assert batch_u0.shape == (args.batch_size, 1, grasp_u.shape[-1])
    # assert batch_x0.shape == (args.batch_size, 1, grasp_x.shape[-1])
    # assert batch_u.shape == (
    #     args.batch_time, args.batch_size, 1, grasp_u.shape[-1])
    # assert batch_x.shape == (
    #     args.batch_time, args.batch_size, 1, grasp_x.shape[-1])
...
if __name__ == '__main__':
    ...
    for iter in range(1, args.niters + 1):
        ...
        for traj in range(num_episode):
            ...
            # Take care of -pi to pi case
            while True:
                diff = batch_x[:-1, :, :, :] - batch_x[1:, :, :, :]

                mask_greater = diff > np.pi
                if mask_greater.any():
                    batch_x[1:, :, :, :] = torch.where(
                        mask_greater,
                        batch_x[1:, :, :, :] + 2 * np.pi,
                        batch_x[1:, :, :, :])
        ...
        if iter % args.test_freq == 0:
            with torch.no_grad():
                ...
                while True:
                    diff = grasp_x_test[:-1, :, :] - grasp_x_test[1:, :, :]

                    mask_greater = diff > np.pi
                    if mask_greater.any():
                        grasp_x_test[1:, :, :] = torch.where(
                            mask_greater,
                            grasp_x_test[1:, :, :] + 2 * np.pi,
                            grasp_x_test[1:, :, :])
                ...
                while True:
                    diff = handover_x_test[:-1, :, :] - \
                        handover_x_test[1:, :, :]

                    mask_greater = diff > np.pi
                    if mask_greater.any():
                        handover_x_test[1:, :, :] = torch.where(
                            mask_greater,
                            handover_x_test[1:, :, :] + 2 * np.pi,
                            handover_x_test[1:, :, :])
                ...
                while True:
                    diff = release_x_test[:-1, :, :] - release_x_test[1:, :, :]

                    mask_greater = diff > np.pi
                    if mask_greater.any():
                        release_x_test[1:, :, :] = torch.where(
                            mask_greater,
                            release_x_test[1:, :, :] + 2 * np.pi,
                            release_x_test[1:, :, :])
